...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        print('drop_path_rate is {}'.format(drop_path_rate))
    ...
    def forward_before_blocks(self, x):
    ...
    def forward(self, x):
        ...
        if self.head_dist is not None:
            head_out, head_out_dist = self.head(x[0]), self.head_dist(x[1])  # x must be a tuple
            return x, (head_out + head_out_dist) / 2
        else:
            out = self.head(x)
            return x, out
